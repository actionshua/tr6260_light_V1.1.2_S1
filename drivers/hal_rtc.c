...
uint32_t rtc_get_time_of_day(void)
{
	...
	nPass4year = (sys_global_time.tm_year - 1970) / 4;
	nYearremaining = (sys_global_time.tm_year - 1970) % 4;

	//计算 距1970 经过年的总天数
	days = nPass4year * 1461 + nYearremaining * 365;
...
#ifdef _USE_RTC_TEST_CMD_READ_TIME
