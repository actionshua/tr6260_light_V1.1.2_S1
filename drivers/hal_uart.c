...
#ifdef HISENSE_LOCK
uint32_t timeout_rx = 5; //band:9600 ref modbus rtu Frame interval 3.5ms, 115200 ?Y?¡§ 5ms
#else
//ms
...
#ifdef HISENSE_LOCK    // uart recv data¡ê?start timer
            hal_timer_config(timeout_rx * 1000, 0);
			hal_timer_start();		
#endif
		}
...
{	
	...
	{		
		...
		xSemaphoreGive(xCountingSemaphore_uart);
#ifndef HISENSE_LOCK	
		hal_timer_start();    //2?¨®??¨´¡ä????¡¥¡ê?¦Ì¨¨?¨®¨º¨¹¦Ì?¡ä??¨²¨ºy?Yo¨®?¨´¡ä????¡¥¡ê?¡¤??¨°¨¤?¡¤??¦Ì¨ª3¡Á¨º?¡ä
#endif	
	}
