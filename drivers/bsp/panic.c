...
void panic_printf(const char *f, ...)
{
    ...
    va_start(ap,f);
    int offset = 0;
    offset += vsnprintf((char *)&print_buffer, PRINT_BUFFER_SIZE+1, f, ap) - 1;
    panic_uart_data_write((unsigned char *)print_buffer, offset);

    if(print_buffer[offset-2] == 0x0a)
    va_end(ap);
}
...
static inline uint32_t gpr(struct pt_regs *pt, int index)
{
	if (index < 6)
		return pt->r[26 + index];
	else
		return pt->r[index - 6];
}
 ...
 */
struct nds32_irq_frame {
#if ((defined(__TARGET_ZOL_EXT) && !defined(__TARGET_IFC_EXT)) || \
     (!defined(__TARGET_ZOL_EXT) && defined(__TARGET_IFC_EXT)))
	uint32_t dummy;
#endif
	void *pc;
	uint32_t psw;
...
{
	uint32_t itype = __nds32__mfsr(NDS32_SR_ITYPE);
	uint32_t faddr __attribute__((unused)) = __nds32__mfsr(NDS32_SR_EVA);
	int inst __attribute__((unused)) = nds32_is_instruction_exception(itype);
...
void nds32_stack_overflow(TaskHandle_t xTask, signed char *pcTaskName)
{
	uint32_t itype;
	struct pt_regs pt, **stack = xTask;

	itype = (__nds32__mfsr(NDS32_SR_ITYPE) & 0xf) | NEXT_PRECISE_STACK_OVERFLOW;
	__nds32__mtsr(itype, NDS32_SR_ITYPE);

	pt = *stack[0];
	pt.sp = (uint32_t) stack[0];
