...
void open_bit_stream(char *filename)
{
  ...
  //  error((char*)"Unable to open output file");
  //bs.b = buff;
  //bs.i = 0;
...
static void putbits (unsigned char *buf, unsigned int val, unsigned int n)
{
  ...
  if (n > 32)
    error((char*)"Cannot write more than 32 bits at a time");

  bits += n;

  while (n)
  {
    k = (n < (unsigned int)bi) ? n : bi;
    tmp = val >> (n - k);
