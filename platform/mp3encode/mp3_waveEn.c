...
void wave_open(int raw, int mono_from_stereo)
{
  ...
  static char const *channel_mappings[] = {NULL,"mono","stereo"};
  static char const *file_type[] = {"RIFF-WAVE PCM","RAW DATA PCM"};
  ...
  wav_read_len += sizeof(header);

  if(strncmp(header.riff,"RIFF",4) != 0) error((char*)"Not a MS-RIFF file");
  if(strncmp(header.wave,"WAVE",4) != 0) error((char*)"Not a WAVE audio");
  if(strncmp(header.fmt, "fmt ",4) != 0) error((char*)"Can't find format chunk");
  if(header.tag != 1)                    error((char*)"Unknown WAVE format");
  if(header.channels > 2)                error((char*)"More than 2 channels");
  if(header.bit_samp != 16)              error((char*)"Not 16 bit");
  if(strncmp(header.data,"data",4) != 0) error((char*)"Can't find data chunk");
  ...
  system_printf("%s, %s %dHz %dbit, Length: %2d:%2d:%2d\n",
          file_type[raw],
...
int wave_get(short buffer[2][1152])
{
  ...
  if (sizeof(wav_data) <= wav_read_len) {
    //system_printf("data done");
    return 0;
  }

  len = (wav_read_len + 2304*sizeof(short)) > sizeof(wav_data) ? sizeof(wav_data) - wav_read_len : 2304*sizeof(short);
  memcpy(temp_buf, wav_data+wav_read_len, len);
  wav_read_len += 2304*sizeof(short);
