...
{
  static char const *mode_names[4]    = { "stereo", "j-stereo", "dual-ch", "mono" };
  static char const *layer_names[4]   = { "", "III", "II", "I" };
  static char const *version_names[4] = { "MPEG 2.5", "", "MPEG 2", "MPEG 1" };
  static char const *psy_names[3]     = { "none", "MUSICAM", "Shine" };
  static char const *demp_names[4]    = { "none", "50/15us", "", "CITT" };
  ...
  system_printf("%s layer %s, %s  Psychoacoustic Model: %s\n",
           version_names[config.mpeg.type],
           layer_names[config.mpeg.layr],
           mode_names[config.mpeg.mode],
           psy_names[config.mpeg.psyc] );
  system_printf("Bitrate=%d kbps  ",config.mpeg.bitr );
  system_printf("De-emphasis: %s   %s %s\n",
 ...
 * -----
 */
int mp3encode_main(int argc, char **argv)
{
  time_t end_time;
  int raw = 0;
...
//config.infile  = argv[i++];
  ...
  system_printf(" Finished in %2ld:%2ld:%2ld\n",
...
#if 1
static int mp3_encode_data(cmd_tbl_t *t, int argc, char *argv[])
{
    uint32_t value = 0;
    ...
    set_defaults();
    wave_open(0, 0); /* prints wave (input) configuration */
    check_config(); /* prints mpeg (output) configuration */
    L3_compress();
    wave_close();
...
#endif
CMD(mp3_encode, mp3_encode_data, "mp3_encode_data", "mp3_encode_data");
