...
/*******************************************/
	...
	memset(host_get, 0, 256);
	strcpy(host_get, host);

	struct sockaddr_in source_addr;
	uint16_t ya_source_port;
	if (flag == 0)
	{
		for (index = 0; index < strlen(host_get); index++)
		{
			if (host_get[index] >= 'A' && host_get[index] <= 'Z')
				host_get[index] = host_get[index] - 'A' + 'a';
		}
	}
...
/*******************************************/
    ...
    if ( getaddrinfo( host_get, port, &hints, &addr_list ) != 0 ) {
        return ( MBEDTLS_ERR_NET_UNKNOWN_HOST );
    }
...
/**********************************************************************/
		...
		system_printf("ya_source_port is: %d\n", ya_source_port);
		...
		if(bind(fd, (struct sockaddr *)&source_addr, sizeof(struct sockaddr_in)) < 0)
		{
			close( fd );
