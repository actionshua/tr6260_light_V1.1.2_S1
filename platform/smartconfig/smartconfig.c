...
static int find_ap_in_aplist(uint8_t * ap_mac, struct ap_info * t_ap)
{
...
int process_manager(void *packet, int size, int8_t rssi)
{
    ...
    ret = ieee80211_get_ssid(mgmt_header, size, ssid);
    if (ret < 0) {
        return -1;
	}

    channel = cfg80211_get_bss_channel(mgmt_header, size);
    rssi = rssi > 0 ? rssi - 256 : rssi;

    cfg80211_get_cipher_info(mgmt_header, size, &auth,
